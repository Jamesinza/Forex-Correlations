...
corr_syms = ['BTCUSD']  #neg_corr_syms
for corr_data in corr_syms:
    ...
    get_data   = get_transformed_data1(corr_data, timeframe, session, num_samples=10_000)
    plain_data = get_data if plain_data is None else np.concatenate([plain_data, get_data], axis=1)
    
    raw_data     = get_real_data_raw(corr_data, timeframe, session, num_samples=10_000)
    ...
    y_data = get_transformed_data1(corr_data, timeframe, session, num_samples=10_000)
    ...
    model = saving.load_model(f'test_models/{corr_data}_{timeframe}_{arch}_{optimizer}_dim{dim}_wl{wl}.keras')
