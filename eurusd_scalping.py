...
from tensorflow.keras import layers, models, callbacks, saving, losses, initializers
...
def create_rnn_base_model(norm, input_shape, dataset, arch,
                          optimizer, dim, seed, dropout, wl, timeframe, transformation):
    # initializer1 = tf.keras.initializers.TruncatedNormal(mean=0.0, stddev=0.02, seed=seed)
    ...
    for _ in range(2):
        x2 = layers.LSTM(dim, return_sequences=True, seed=seed, kernel_initializer=glorot,
                         recurrent_initializer=orthog)(x2)
    ...
    # for units in [dim, dim*2, dim]:
    #     out = layers.Dense(units, activation='tanh', kernel_initializer=initializer2)(out)
        
    output = layers.Dense(1, kernel_initializer=initializer2)(out)
    model = models.Model(inputs, output, name=f'{dataset}_{timeframe}_{arch}_{optimizer}_dim{dim}_wl{wl}_{transformation}')
    model.compile(optimizer=optimizer, loss=losses.Huber(delta=1.0),
...
def TCNBlock(x, filters, kernel_size, dilation_rate, dropout, seed):
    ...
    conv1 = layers.Conv1D(filters, kernel_size, padding="causal",
                          dilation_rate=dilation_rate, kernel_initializer=initializer)(x)
...
for corr_data in corr_syms:
    ...
    for transformation in transformations:
        ...
        batch_size  = 256 #compute_batch_size(len(train_data))
        ...
        norm = None #tf.keras.layers.Normalization()
        # norm.adapt(adapt_ds)
        ...
        #                                   optimizer, dim, seed, dropout, wl, timeframe, transformation)
        model = create_rnn_base_model(norm, input_shape, corr_data, arch,
                          optimizer, dim, seed, dropout, wl, timeframe, transformation)
        ...
        model.save(f'test_models/{corr_data}_{timeframe}_{arch}_{optimizer}_dim{dim}_wl{wl}_{transformation}_v3.keras')
