...
from tensorflow.keras import layers, models, callbacks, saving, optimizers, losses
from keras_hub.layers import FNetEncoder, SinePositionEncoding
from sklearn.utils.class_weight import compute_class_weight


def create_dataset(X, target, wl, batch_size, shuffle=False):
    data = X  #np.hstack([X, y])
    print(f'\n data shape for tf.data: {data.shape}\n')
    dataset = tf.data.Dataset.from_tensor_slices(data)
    dataset = dataset.window(wl + 1, shift=1, drop_remainder=True)
    dataset = dataset.flat_map(lambda window: window.batch(wl + 1))
    dataset = dataset.map(lambda window: (window[:-1], window[-1:, target]))
    if shuffle==True:
...
def process_data(X_train_data, target, X_val_data, wl, batch_size):
    train_ds = create_dataset(X_train_data, target, wl, batch_size, shuffle=False)
    val_ds = create_dataset(X_val_data, target, wl, batch_size, shuffle=False)
...
def create_rnn_base_model(norm, input_shape, dataset, arch,
    ...
    model.compile(optimizer=optimizer, loss=losses.Huber(delta=0.5), metrics=['mae','mse'], jit_compile=False)
    return model    
...
def create_att_base_model(norm, input_shape, dataset, arch,
    ...
    model = models.Model(inputs, output, name=f'{dataset}_{arch}_{optimizer}_dim{dim}_seed{seed}_s{s}')
    model.compile(optimizer=optimizer, loss='huber', metrics=['mae','mse'], jit_compile=True)
    return model    
...
def create_tsmixer_base_model(norm, input_shape, wl, dataset, arch, optimizer,
    ...
    output = layers.Dense(1)(out)
    model = models.Model(inputs, output, name=f'{dataset}_{arch}_{optimizer}_dim{dim}_seed{seed}_s{s}')
    model.compile(optimizer=optimizers.Adam(5e-04), loss='huber', metrics=['mae','mse'], jit_compile=True)
    return model
...
def create_tcn_base_model(norm, input_shape, dataset, arch,
    ...
    # out =  layers.GlobalAveragePooling1D()(x)
    out = layers.Reshape((x.shape[1]*x.shape[2],), name='flatten')(x)
    # out = x
    ...
    output = layers.Dense(1)(out)
    model = models.Model(inputs, output, name=f'{dataset}_{arch}_{optimizer}_dim{dim}_seed{seed}_s{s}')
    model.compile(optimizer=optimizer, loss=losses.Huber(delta=1.5), metrics=['mae','mse'], jit_compile=True)
    return model    
...
def trainer(input_shape, norm, train_ds, val_ds, dataset, arch, optimizer, batch_size,
    ...
    if arch=='tsmixer':
        ...
        model = create_rnn_base_model(norm, input_shape, dataset, arch,
                                      optimizer, dim, seed, dropout, num_classes, s)
    elif arch=='tcn':
    ...
    model.fit(train_ds, epochs=epochs, batch_size=batch_size, validation_data=val_ds,
              verbose=1, validation_steps=val_steps, callbacks=callback, class_weight=class_weights_dict,
...
def main():
    dataset     = 'BTCUSD_M15_245' #sys.argv[1]
    sub_folder  = 'M1' #sys.argv[3]
    arch        = 'tcn' #sys.argv[4]
    wl          = 8 #int(sys.argv[5])
    ...
    class_weights_dict = None #dict(enumerate(class_weights))
    num_classes        = None #len(unique_classes)
    
    train_ds, val_ds = process_data(X_train_data, target, X_val_data,
                                    wl, batch_size)
    ...
    # X_val = np.empty([len(X_val_data)-wl, wl], dtype=np.float32)
    # y_val = np.empty([len(X_val_data)-wl], dtype=np.float32)
    # for i in range(len(X_val_data)-wl):
    #     X_val[i] = X_val_data[i:i+wl]
    #     y_val[i] = y_val_data[i+wl]
    # model = saving.load_model(f'test_models/{sub_folder}/{dataset}_{arch}_{optimizer}_dim{dim}_seed{seed}_s{s}_wl{wl}.keras', compile=True)
