...
corr_syms = ['EURUSD']  #neg_corr_syms
for corr_data in corr_syms:
    ...
    plain_data = get_data.copy()    
    
    raw_data     = get_real_data_raw(corr_data, timeframe, session, num_samples=num_samples)
    raw_val_data = raw_data #[int(len(raw_data)*0.95):]
    
    features    = plain_data.shape[-1]
    input_shape = (wl, features)
    dim         = features
    
    y_data = get_data.copy()  #get_transformed_data1(corr_data, timeframe, session, num_samples=num_samples)
    
    data = np.concatenate([plain_data, y_data.reshape(-1,1)], axis=1)
    
    val_data   = data #[int(len(data)*0.95):]
   
    X_test = np.empty([len(val_data)-wl, wl, features], dtype=np.float32)
    y_test = np.empty([len(val_data)-wl, 1], dtype=np.float32)
    for i in range(len(val_data)-wl):
        X_test[i] = val_data[i:i+wl, :-1]
        y_test[i] = val_data[i+wl, -1]
    
    model = saving.load_model(f'test_models/{corr_data}_{timeframe}_{arch}_{optimizer}_dim{dim}_wl{wl}_{transformation}_stage2.keras')
    
    preds = model.predict(X_test, verbose=0)
    
    nums = 5000
    preds_true_compare = np.hstack([preds[-nums:], y_test[-nums:]])
    
    lst = []
    for i in range(len(y_test[-nums:])):
        lst.append((preds_true_compare[i]<0)[0]==(preds_true_compare[i]<0)[1])
    
    res, cnt = np.unique(lst, return_counts=True)
    print(f'\nResults for {corr_data}:\n{res[0]} = {cnt[0]}\n{res[1]}  = {cnt[1]}')
    tf.keras.backend.clear_session()

# M1 testing
test_data = X_test[0].copy()
raw = raw_val_data[96:].copy()
checks = 20
steps = 60
num_checks = checks * steps
for j in range(0,num_checks,steps):
    print(f'\nStarting at: {j}')
    start = j
    end   = start + steps
    true  = []
    for i in range(start, end):
        a = raw[i]  #(raw_val_data[96:-1:])[i]
        b = y_test[i]

        # print(a, '\t', b)
        
        res1 = a + (a*b) if transformation=='pct' else a * np.exp(b)
        true.append(res1)
    # print(np.array(true).flatten())
    print(f'\nTrue Result: {(true[0]-true[-1])[0]:.5f}\tDrawdown: { ((np.max(true)-true[0])[0] if
                                                                     (((preds[0]-preds[-1])<0) and ((true[0]-true[-1])<0)) else
                                                                     (true[0]-np.min(true))[0] if (((preds[0]-preds[-1])>0) and ((true[0]-true[-1])>0)) else 0):.5f}')
    
    # print('\n')
    preds = []
    for i in range(start, end):
        c = model.predict(test_data.reshape(-1,96,1), verbose=0)
        preds.append(raw[i].flatten() + (raw[i] * c).flatten())  if transformation=='pct' else preds.append((raw[i].flatten() * np.exp(c)).flatten())
        test_data = np.vstack([test_data[1:], c.reshape(-1,1)])
    # print(np.array(preds).flatten())
    print(f'Pred Result: {(preds[0]-preds[-1])[0]:.5f}\tDrawdown: {((np.max(preds)-preds[0])[0] if
                                                                      (((preds[0]-preds[-1])<0) and ((true[0]-true[-1])<0)) else
                                                                      (preds[0]-np.min(preds))[0] if (((preds[0]-preds[-1])>0) and
                                                                      ((true[0]-true[-1])>0)) else 0):.5f}')
    print(f'Match : {((((preds[0]-preds[-1])<0) and ((true[0]-true[-1])<0)) or (((preds[0]-preds[-1])>0) and ((true[0]-true[-1])>0))).flatten()[0]}')
    print(f'Action: {1 if (abs(preds[0]-preds[-1])>5e-4) else 0}\n\n')    